...
# Configuration
$MaxDepth = 0  # 0 = unlimited depth (show everything)
$CollapseThreshold = 0  # 0 = don't collapse (show all files)
$ShowFirstFiles = 999  # Show all files
$AutoExclude = $false  # Don't exclude anything by default
...
$htmlLines += '<div class="controls"><div class="search-box"><input id="search" type="text" placeholder="Search files and folders..."></div><button id="smartViewBtn">Smart View (Hide System Files)</button><button id="expandBtn">Expand All</button><button id="collapseBtn">Collapse All</button><button id="copyBtn">Copy as Text</button></div>'
...
$htmlLines += 'var smartViewEnabled=false;'
$htmlLines += 'var systemFolders=["node_modules",".git",".vscode",".idea",".vs","dist","build","out","bin","obj","__pycache__",".cache",".tmp",".temp"];'
$htmlLines += 'var systemFiles=[".DS_Store","Thumbs.db","desktop.ini"];'
$htmlLines += 'function shouldHideInSmartView(name){if(!smartViewEnabled)return false;for(var i=0;i<systemFolders.length;i++){if(name===systemFolders[i])return true}for(var i=0;i<systemFiles.length;i++){if(name.endsWith(systemFiles[i]))return true}if(name.endsWith(".log"))return true;return false}'
$htmlLines += 'function renderTree(){var html="";function render(node,depth){if(!node)return"";if(shouldHideInSmartView(node.name))return"";var id="n"+Math.random().toString(36).substr(2,9);var indent="<span style=\"display:inline-block;width:"+(depth*20)+"px\"></span>";var line="<div class=\"tree-item\" data-id=\""+id+"\">";if(node.type==="folder"){var hasKids=node.children&&node.children.length>0;line+=indent;if(hasKids){line+="<span class=\"toggle\" data-nodeid=\""+id+"\">+</span>"}else{line+="<span class=\"toggle\"> </span>"}line+="<span class=\"tree-folder\">[+] "+escapeHtml(node.name)+"/</span>";if(node.fileCount>0){line+="<span class=\"tree-size\">("+formatSize(node.size)+" - "+node.fileCount+" files)</span>"}line+="</div>";if(hasKids){line+="<div class=\"tree-children\" id=\""+id+"\">";node.children.forEach(function(child){line+=render(child,depth+1)});line+="</div>"}}else if(node.type==="file"){line+=indent+"<span class=\"toggle\"> </span>";var cls=node.important?"tree-important":"tree-file";line+="<span class=\""+cls+"\">[-] "+escapeHtml(node.name);if(node.important)line+=" [*]";line+="</span>";line+="<span class=\"tree-size\">"+formatSize(node.size)+"</span>";line+="</div>"}else{line+=indent+"<span class=\"toggle\"> </span><span class=\"tree-file\">"+escapeHtml(node.name)+"</span></div>"}return line}html=render(treeData,0);document.getElementById("tree").innerHTML=html}'
$htmlLines += 'function escapeHtml(text){var div=document.createElement("div");div.textContent=text;return div.innerHTML}'
...
$htmlLines += 'document.getElementById("smartViewBtn").addEventListener("click",function(){smartViewEnabled=!smartViewEnabled;this.textContent=smartViewEnabled?"Full View (Show All)":"Smart View (Hide System Files)";this.style.background=smartViewEnabled?"#28a745":"#007bff";renderTree()});'
...
$htmlLines += 'function formatSize(bytes){if(bytes===0)return"0 B";if(bytes<1024)return bytes+" B";if(bytes<1048576)return(bytes/1024).toFixed(2)+" KB";if(bytes<1073741824)return(bytes/1048576).toFixed(2)+" MB";return(bytes/1073741824).toFixed(2)+" GB"}'
$htmlLines += 'document.getElementById("copyBtn").addEventListener("click",function(){var text="";function build(node,depth,isLast,prefix){if(!node)return;if(shouldHideInSmartView(node.name))return;var indent=prefix;var connector=isLast?"+-- ":"|-- ";if(node.type==="folder"){text+=indent+connector+node.name+"/";if(node.fileCount>0)text+=" ("+formatSize(node.size)+" - "+node.fileCount+" files)";text+="\n";if(node.children){var visibleChildren=node.children.filter(function(c){return!shouldHideInSmartView(c.name)});var newPrefix=indent+(isLast?"    ":"|   ");visibleChildren.forEach(function(child,idx){build(child,depth+1,idx===visibleChildren.length-1,newPrefix)})}}else if(node.type==="file"){text+=indent+connector+node.name;if(node.important)text+=" [*]";text+=" ("+formatSize(node.size)+")\n"}else{text+=indent+connector+node.name+"\n"}}build(treeData,0,true,"");navigator.clipboard.writeText(text).then(function(){alert("Tree structure copied to clipboard!\n\nYou can now paste it into Claude or any AI.")}).catch(function(){var textarea=document.createElement("textarea");textarea.value=text;textarea.style.position="fixed";textarea.style.opacity="0";document.body.appendChild(textarea);textarea.select();document.execCommand("copy");document.body.removeChild(textarea);alert("Tree structure copied to clipboard!\n\nYou can now paste it into Claude or any AI.")})});'
